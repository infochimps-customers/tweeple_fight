class Main
  helpers do
    ...
    def form_tag path, options={}, &block
      options.merge! :action => path
      haml_tag :form, options, &block
    end
    ...
    def labeled_field obj, field, options={}, &block
      haml_tag :div, :class => 'form_input' do
        label(obj, field)
        text_field(obj, field, options)
        yield if block
      end
    ...
    def label(obj, field, label_text="", options={}, &block)
      text = label_text.blank? ? field.to_s.humanize : label_text
      options.merge!(:for => form_obj_id(obj, field))
      haml_tag :label, text, options, &block
    end
    ...
    def checkbox obj, field, value, label_text="", options={}
      options = options.merge(:checked => true) if obj.send(field).to_s == value.to_s
      haml_tag :input, options.reverse_merge(:type => "hidden",   :value => '0',   :name => form_obj_name(obj, field) )
      haml_tag :input, options.reverse_merge(:type => "checkbox", :value => value, :name => form_obj_name(obj, field), :id => form_obj_id(obj, field) )
      label obj, field, label_text
    end
